...
<div class="content content--top-nav">
    ...
    <div class="intro-y box col-span-12 lg:col-span-6 mt-5">
        <div id="vertical-form" class="p-5">
            <form class="preview" action="{{route('master.desa-kelurahan.update')}}" method="POST"
                enctype="multipart/form-data">
                @csrf
                <div class="mt-3">
                    <label for="input-wizard-2" class="sm:w-40 font-bold">Kecamatan</label>
                    <select data-placeholder="Pilih Status" class="tom-select w-full form-control mt-2" name="kecamatan_id" required>
                        <option value=" "> - </option>
                        @foreach($kecamatans as $kecamatan)
                        <option value="{{$kecamatan->id}}" <?php  echo ($kecamatan->id == $data->kecamatan_id) ? 'selected' : ''; ?>>{{$kecamatan->nama}}</option>
                        @endforeach
                    </select>
                ...
                <div class="mt-3 text-right">
                    <a href="{{route('master.desa-kelurahan.index')}}" class="btn btn-danger mt-5">Kembali</a>
