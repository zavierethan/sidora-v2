...
<div class="content content--top-nav">
    ...
    <div class="intro-y box col-span-12 lg:col-span-6 mt-5">
        <div id="vertical-form" class="p-5">
            <form class="preview" action="{{route('transaksi.pengaturan.user.update')}}" method="POST" enctype="multipart/form-data">
                @csrf
                <div>
                ...
                <div class="mt-3">
                    ...
                    <select data-placeholder="Pilih Status" class="tom-select w-full form-control mt-2" name="role_id" required>
                        <option value=" "> - </option>
                        @foreach($roles as $role)
                        <option value="{{$role->id}}" <?php  echo ($role->id == $data->role_id) ? 'selected' : ''; ?>>{{$role->name}}</option>
                        @endforeach
                    </select>
                </div>
                <div class="mt-3">
                    <label for="input-wizard-2" class="sm:w-40 font-bold">Status</label>
                    <select data-placeholder="Pilih Status" class="tom-select w-full form-control mt-2" name="status" required>
                        <option value=" "> - </option>
